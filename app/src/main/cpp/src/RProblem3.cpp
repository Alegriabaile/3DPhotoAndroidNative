...
int RProblem3::point_transfer(const cv::Vec3f& point0, const cv::Vec3f& rvec, const cv::Vec3f& tvec, cv::Vec3f& point1)
{
...
int RProblem3::depth_transfer(const double & scale, const double & offset, cv::Mat& depth)
{
    ...
    depth = float(scale)*depth+float(offset);
...
int RProblem3::updateParameters(std::vector<i3d::Frame> & kframes)
{
    //更新模型矩阵
...
RProblem3::RProblem3(std::vector<i3d::Frame> &kframes, std::vector<i3d::Edge> &kedges,
...
int RProblem3::initParameters(const std::vector<i3d::Frame> &kframes,
...
public:
                    ...
                    T* residuals) const
    {
        ...
        T a_d_ =  (scale_weight*T(a_d) + offset_weight);
