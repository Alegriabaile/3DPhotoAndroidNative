...
int DProblem1::point_transfer(const cv::Vec3f& point0, const cv::Vec3f& rvec, const cv::Vec3f& tvec, cv::Vec3f& point1)
{
...
int DProblem1::depth_transfer(const double * const scales, const double * const offsets, cv::Mat& depth)
{
#ifdef DEPTH_TRANSFER_DEBUG
    using namespace std;
    cout<<endl;
    for(int j=0; j<((grid_w>1 && grid_h>1)?grid_w*grid_h:1); ++j)
        cout<<scales[j]<<" ";//
    cout<<endl;
    for(int j=0; j< ((grid_w>1 && grid_h>1)?grid_w*grid_h:1); ++j)
        cout<<offsets[j]<<" ";//
    ...
    cout<<endl<<"original min, max: "<<minVal<<", "<<maxVal<<endl;
    ...
    for(int h = 0; h < rows; ++h)
    {
        for(int w = 0; w < cols; ++w)
        {
            ...
            double scale = scales[k0]*w0 + scales[k0+1]*w1 + scales[k0+grid_h]*w2 + scales[k0+grid_h+1]*w3;
            double offset = offsets[k0]*w0 + offsets[k0+1]*w1 + offsets[k0+grid_h]*w2 + offsets[k0+grid_h+1]*w3;

            double d = depth.at<float>(h, w);
            double d_new = d/(scale+offset*d);//转换公式
...
int DProblem1::updateParameters(std::vector<i3d::Frame> & kframes)
{
    //更新模型矩阵
...
DProblem1::DProblem1(std::vector<i3d::Frame> &kframes, std::vector<i3d::Edge> &kedges,
                     i3d::Intrinsics &intrinsics, int gridw, int gridh, int iter_num)
...
int DProblem1::initParameters(const std::vector<i3d::Frame> &kframes,
                              const std::vector<i3d::Edge> &edges, const i3d::Intrinsics &intrinsics)
{
    ...
    for(int i=0; i<edges.size(); ++i)
    {
        ...
        for(int k=0; k<edges[i].psrc.size(); ++k)
        {
            ...
            temp_match.ad = kframes[edges[i].src].depth.at<double >
            ...
            temp_match.bd = kframes[edges[i].dst].depth.at<double >
...
public:
                    ...
                    T* residuals) const
    {
        ...
        scale_weight = scales_a0[0]*w0 + scales_a1[0]*w1 + scales_a2[0]*w2 + scales_a3[0]*w3;
        offset_weight = offsets_a0[0]*w0 + offsets_a1[0]*w1 + offsets_a2[0]*w2 + offsets_a3[0]*w3;
    ...
    {
        return (new ceres::AutoDiffCostFunction<D2CostFunctor1, 2, 6, 6, 1, 1, 1, 1, 1, 1, 1, 1>(
                new D2CostFunctor1(a_x, a_y, a_d, b_x, b_y, intrinsics, w0, w1, w2, w3)));
    }
...
int DProblem1::solveProblem()
{
    ...
    for (int i = 0; i < num_matches; ++i)
    {
        ...
        colorw = this->intrinsics.cx * 2;
        colorh = this->intrinsics.cy * 2;
        int k0;
        double w0, w2;
        double w1, w3;
        k0 = bilinearWeight(a_x, a_y, colorw, colorh, w0, w1, w2, w3);
        if (k0 == -1)//global affine alignment
        {
            std::cout<<"grid_h<=1 || grid_w<=1!!!"<<std::endl;
            return -1;
        } else//grid affine alignment
        {
            ...
            problem.AddResidualBlock(costFunctor1_0,
                                     ...
                                     scales + k0, scales + k0 + 1, scales + k0 + grid_h, scales + k0 + grid_h + 1,
                                     offsets + k0, offsets + k0 + 1, offsets + k0 + grid_h, offsets + k0 + grid_h + 1);

            //主要为了求解B图像的scales、offsets因子
            k0 = bilinearWeight(b_x, b_y, colorw, colorh, w0, w1, w2, w3);
    ...
    for (int c = 0; c < num_cameras; ++c)
    {
        ...
        {
            for (int h = 0; h < grid_h - 1; ++h)
            {

                int k = w * grid_h + h;
                ceres::CostFunction *costFunctor2_h = D2CostFunctor2::Create(l1);
                problem.AddResidualBlock(costFunctor2_h, NULL,
                                         scales + k, scales + k + 1,
                                         offsets + k, offsets + k + 1);
            }
        }
        for (int w = 0; w < grid_w - 1; ++w)
        {
            for (int h = 0; h < grid_h; ++h)
            {
                int k = w * grid_h + h;
                ceres::CostFunction *costFunctor2_w = D2CostFunctor2::Create(l1);
                problem.AddResidualBlock(costFunctor2_w, NULL,
                                         scales + k, scales + k + grid_h,
                                         offsets + k, offsets + k + grid_h);
            }
        }

        //Escale
        double l2 = 1e-2 * depth_expande;//1e-2, 5,
