...
namespace i3d_rp {
    ...
    public:
        ...
        RigidProblem(std::vector<i3d::Frame> &kframes, std::vector<i3d::Edge> &kedges,
                     i3d::Intrinsics &intrinsics)
                : num_matches_(0), num_cameras_(0), camera_i_(nullptr), camera_j_(nullptr),
                  matches_(nullptr), parameters_(nullptr) {
...
//////////////////////////模板代码不能放到cpp文件中，因为其模板参数必须在编译时给出，而cpp文件在编译时已经被编译为源代码////////////////////
    ...
    int RigidProblem::point_transfer(const cv::Vec3f &point0, const cv::Vec3f &rvec,
...
////////////about ceres functor and solver////////////////////////////
    ...
    public:
        ...
        CostFunctor1(
                ...
                i3d::Intrinsics intrinsics)
                : a_x(a_x), a_y(a_y), a_d(a_d), b_x(b_x), b_y(b_y), b_d(b_d),
    ...
    int RigidProblem::solveProblem() {
        ...
        for (int k = 0; k < num_matches; ++k) {
            ...
            ceres::CostFunction *cost_function =
                    CostFunctor1::Create(*(matches + 6 * k), *(matches + 6 * k + 1),
                                         *(matches + 6 * k + 2),
                                         *(matches + 6 * k + 3), *(matches + 6 * k + 4),
                                         *(matches + 6 * k + 5), this->intrinsics);
