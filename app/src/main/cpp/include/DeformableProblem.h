...
public:
    ...
    DeformableProblem(std::vector<i3d::Frame> & kframes, std::vector<i3d::Edge> & kedges, i3d::Intrinsics & intrinsics)
            : num_matches_(0), num_cameras_(0)
            , camera_i_(nullptr), camera_j_(nullptr)
            , matches_(nullptr), parameters_(nullptr)
            , scales_(nullptr), offsets_(nullptr)
...
//        LOGE("insert():: finish...");
    ...
    int bilinearWeight(double w, double h, double cols, double rows, double& w0, double&w1, double& w2, double& w3)
    {
...
template <int gridw, int gridh>
int DeformableProblem<gridw, gridh>::point_transfer(const cv::Vec3f& point0, const cv::Vec3f& rvec, const cv::Vec3f& tvec, cv::Vec3f& point1)
{
...
template <int gridw, int gridh>
int DeformableProblem<gridw, gridh>::depth_transfer(const double * const scales, const double * const offsets, cv::Mat& depth)
{
...
public:
    ...
    CostFunctor1(
            ...
            , cx(intrinsics.colorw/2), cy(intrinsics.colorh/2)
...
struct CostFunctor2{
    ...
    {
        return ( new ceres::AutoDiffCostFunction < CostFunctor2, 2, 1,1,1,1> ( new CostFunctor2(l1)));
    }
private:
    double l1;
};
...
int DeformableProblem<gridw, gridh>::solveProblem()
{
    ...
    for (int i = 0; i < num_matches; ++i)
    {
        double a_x, a_y, a_d;
        double b_x, b_y, b_d;
        a_x = *(matches+6*i); a_y = *(matches+6*i+1); a_d = *(matches+6*i+2);
        b_x = *(matches+6*i+3); b_y = *(matches+6*i+4); b_d = *(matches+6*i+5);
    ...
    //l1, l2最终会经过平方
    //目前只适用于kinect深度
    const double depth_expande = sqrt(25*450/double(gridw*gridh)/double(num_matches));//*num_matches/500.0);//*num_matches/500.0);//4/gridw/gridh;//sqrt(4*num_matches/1000/gridw/gridh);//sqrt(i3d::MAX_DEPTH_VALUE*4*num_matches/1000/gridw/gridh);
