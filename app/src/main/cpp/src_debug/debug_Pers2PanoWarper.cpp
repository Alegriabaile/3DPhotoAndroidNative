...
int main(int argc, char** argv)
{
    ...
    //DProblem1 deformableProblemWxh(kframes, kedges, intrinsics, 2, 2, 50);
    for(int i=0; i<kframes.size(); ++i)
    {
        cout<<i<<" th rx,ty,tz, tx,ty,tz: "
            <<kframes[i].rx<<","<<kframes[i].ry<<","<<kframes[i].rz<<", "
    ...
    for(int i=0; i<kframes.size(); ++i)//(kframes.size()>17?10:kframes.size()); ++i)
    {
        cv::Mat R;
    ...
    {
        double minD, maxD;
        minMaxLoc(kframes[i].pano_depth, &minD, &maxD);
        //cout<<i<<" th frame depth, min, max:"<<minD<<", "<<maxD<<endl;
        //imshow("warped pano_depth", kframes[i].pano_depth);
        //waitKey();
    }*/
    return 0;
}