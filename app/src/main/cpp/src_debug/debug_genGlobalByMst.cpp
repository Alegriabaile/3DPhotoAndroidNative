...
int main()
{
    ...
    for(int i=0; i<frames.size() && false; ++i)
    {
        double minVal, maxVal;
        minMaxLoc(frames[i].depth, &minVal, &maxVal);
        cout<<i<<" th min, max: "<<minVal<<", "<<maxVal<<endl;
        imshow("image", frames[i].image);
        imshow("depth", (frames[i].depth-minVal)/(maxVal-minVal));
        cout<<frames[i].depth;
        waitKey();
    }
    ...
    {
        cout<<"edges[i].src, dst: "<<edges[i].src<<", "<<edges[i].dst<<endl;
    }
    for(int i=0; i<kedges.size(); ++i)
    {
        cout<<"kedges[i].src, dst: "<<kedges[i].src<<", "<<kedges[i].dst<<endl;
    }
//debug
    cout<<"debug_genRts.cpp: generate pcl data"<<endl;
    PointCloud<PointXYZRGBA>::Ptr out_cloud( new PointCloud<PointXYZRGBA>);
    for(int i=0; i<(kframes.size()>17?10:kframes.size()); ++i)
    {
        cv::Mat R;
        ...
        //backprojectToPointXYZRGBA(frames[i].image, frames[i].depth, K, cloud1);
        PointCloud<PointXYZRGBA>::Ptr cali_cloud1(new PointCloud<PointXYZRGBA>);
        BackprojectToPano(kframes[i], intrinsics, cali_cloud1);
        //pcl::transformPointCloud( *cloud1, *cali_cloud1, T.matrix() );
        *out_cloud += *cali_cloud1;
        //cloud1->clear();
        cali_cloud1->clear();

    }
    cout<<"debug_genRts.cpp: cloud viewer to show..."<<endl;
